...
module.exports = ({ config, db, router, cache, apiStatus, apiError, getRestApiClient }) => {
    const createMage2RestClient = () => {
        ...
        client.addMethods('multiwishlist', (restClient) => {
            const module = {};
            module.getMultiwishlists = ({ customerId, withItems }, token) => {
                const url = `/kmk-multi-wishlist/wishlist/customer/${customerId}?withItems=${withItems || false}`;
                ...
                .then(async response => {
                    if (response && response instanceof Array) {
                        ...
                        if (single.hasOwnProperty('items') && single.items instanceof Array) {
                            ...
                            const items = await decorator.decorateProducts(single.items, 'product_id', 'price');
    ...
    router.get('/:customerId', (req, res) => {
        ...
        const { token, withItems } = req.query;
        const additionalParams = req.query;
        const client = createMage2RestClient();
        try {
            if (!customerId) { throw new Error('Customer id is required'); }
            client.multiwishlist.getMultiwishlists({ customerId, withItems }, token)
                .then(response => apiStatus(res, response, 200))
                .catch(err => apiError(res, err));
        } catch (e) {
            apiError(res, e);
